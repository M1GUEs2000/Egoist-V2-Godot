...
| Espada | `WeaponBase` · `Sword` · `SwordTuning` | **E2** | Regresion por variantes RT de taps X y continuidad tap → carga (2026-07-28): falta re-probar jugando brillo, bloqueo del cargado hasta cerrar vueltas/Mover, proyectiles y alturas. Clash mid-swing y ultimos detalles de mano orbital siguen pendientes |
| Mazo | `WeaponBase` · `Mace` · `MaceTuning` | **E2** | Reconstruido (2026-07-21) sobre Mover/Floater: Y cargado aereo quitado (dependia de un bouncer balistico sin diseñar), launcher terrestre corregido (elevaba nada por un `enemy_mover` en null). Combos completos sobre el mismo motor que la Espada; falta probarse jugando (E2→E3 lo decide Tutupa) |
