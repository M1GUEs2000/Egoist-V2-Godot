...
| Player — movimiento | `Player` · `PlayerLocomotion` · `PlayerDash` · `PlayerLauncher` · `PlayerWallSlide` · `PlayerEnemyBounce` | **E3** | Wall slide reworkeado (assist sin depender del stick + gracia coyote + arco genuino por gravedad reducida simétrica) y aprobado jugando; faltan juice y edge cases. Dash/launcher/rebote siguen pendientes de validar jugando |
