...
| Player — movimiento | `Player` · `PlayerLocomotion` · `PlayerDash` · `Mover` · `Floater` · `PlayerWallSlide` · `PlayerEnemyBounce` | **E3** | Aprobado jugando: movimiento base, wall slide/Wall Impulse (carril que sigue curvas, inclinable y tuneable por pared) y control vertical con Mover/Floater. Dash/airdash llegó a **E4** en su subnota (`Dash y Airdash`). Faltan juice y edge cases; el rebote en enemigos queda como la pieza más verde del bundle |
