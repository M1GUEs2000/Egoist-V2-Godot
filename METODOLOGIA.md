...
| Player — movimiento | `Player` · `PlayerLocomotion` · `PlayerDash` · `PlayerWallSlide` · `PlayerEnemyBounce` · `Floater` · `Mover` | **E3** | Aprobado jugando: movimiento base, wall slide/Wall Impulse (carril que sigue curvas, inclinable y tuneable por pared) y launcher. Dash/airdash llegó a **E4** en su subnota (`Dash y Airdash`). Faltan juice y edge cases; el rebote en enemigos queda como la pieza más verde del bundle. *(2026-07-20)* `PlayerLauncher` **eliminado**: ya no lanzaba (launch → `Mover` en F2) ni sostenía hang (stall/hover → `Floater` en F3); su contabilidad de air-hit-stall + whiff se plegó dentro de `Player`. Refactor sin cambio de comportamiento (verificado headless) — **confirmar jugando** que el feel del combo aéreo no cambió antes de dar por firme el E3. *(2026-07-20)* **Dash E4→E3**: el X cargado de la Espada dejó de usar `PlayerDash.force_dash` y pasó a un `Mover` EXCLUSIVO con los extras portados (pass-through, boost, partículas, inercia de salida vía hooks `on_mover_started/ended`). El plunge pasó a `Mover` DOWN no-exclusivo (se borró `_plunge_speed`). Confirmar jugando dash cargado y plunge antes de volver a E4/E3 |
...
| Player — meter | `PlayerMeter` | **E3** | Aprobado jugando; faltan juice y edge cases. Mejoras futuras (5 barras, esquive perfecto) quedan como diseño pendiente |
...
| Espada | `WeaponBase` · `Sword` · `SwordTuning` | **E2** | Regresó de E3 *(2026-07-20)*: migración a Mover/Floater — plunge (`_player.plunge` → Mover DOWN no-exclusivo), X cargado (→ Mover con extras portados) y Y cargado aéreo DESACTIVADO (esperando el bouncer). Clash mid-swing pendiente (ponytail) y últimos detalles de mano orbital. Reprobar jugando dash cargado, plunge y que el Y aéreo caído al combo normal se sienta bien |
