...
| Espada | `WeaponBase` · `Sword` · `SwordTuning` · `AttackMovementProfile` | **E2** | Refactor (2026-07-29): los taps de X pasaron a un `AttackMovementProfile` por variante — se fueron 11 campos sueltos del tuning y 7 ids de rutina de `Sword`. **Commiteado sin verificacion headless**: correr `--import`, `--quit-after 2` y los dos smokes antes de seguir. Un cambio de feel deliberado: los RT aereos, que dejaban al enemigo sin hang, ahora caen al hold generico de 0.2 s. Los taps de Y no migraron. Ademas, dos regresiones previas (2026-07-28). Variantes RT de taps X y continuidad tap → carga: falta re-probar jugando brillo, bloqueo del cargado hasta cerrar vueltas/Mover, proyectiles y alturas. Y su Y cargado terrestre (launcher) pasó a costar 1 barra —era el único cargado gratis del arma— y sin barra cae al tap normal: falta jugar si sigue valiendo la pena a ese precio. Clash mid-swing y ultimos detalles de mano orbital siguen pendientes |
