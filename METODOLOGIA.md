...
| Player — movimiento | `Player` · `PlayerLocomotion` · `PlayerDash` · `PlayerLauncher` · `PlayerWallSlide` · `PlayerEnemyBounce` | **E2** | Regresión E3→E2 por control aéreo con inercia (`air_acceleration`: en el aire el stick empuja la velocidad conservada, ya no manda directo): falta probar jugando. Wall slide aprobado jugando; dash/launcher/rebote pendientes de validar jugando |
| Floor slide por plataforma | `PlayerFloorSlide` · `FloorSlideSurface` · `FloorSlideTuning` | **E0** | Recién construido: slide de suelo por plataforma (hielo/rampa con una sola fórmula), inyectable como el wall slide. Correr headless/probe (`floor_slide_probe`) y tunear jugando fricción, pendiente, `steer_control` y `jump_momentum_keep`. Los knobs existen pero la dirección del feel es desconocida |
...
| Cámara | `CameraRig` · `CameraTuning` | **E1** | Rotacion horizontal por stick libre (360°, sin clamp, sin recentrado automatico); falta headless + tunear jugando `yaw_speed` y si la falta de recentrado se siente bien; falta definir centro por area |
| Lock-on | `LockOn` · `CameraRig` (modo lockeado) | **E2** | Boton dedicado (`C`) tipo Dark Souls con ciclado por Q/E; en modo lockeado la camara ya no orbita a la espalda del jugador (mantiene el yaw libre que tenia) y hace zoom in/out de distancia segun separacion jugador-target (`lock_zoom_*`). Falta headless + iterar jugando `lock_focus_weight` y el nuevo rango de zoom (E2→E3 pendiente de aprobacion final) |
| Brazo (puño remoto, combate + traversal) | `PlayerArm` · `ArmTuning` · `TraversalBlock` | **E2** | Combate: golpea al target del lock-on pasivo con daño/poise/meter propios y bajos, 5 taps seguidos + cooldown de 3s. Traversal (recién agregado): si no hay enemigo en el cono, marca el bloque de dash verde más cercano en su propio cono/rango y lo activa empujando al jugador con un dash forzado (`teleport_duration`) + cooldown propio de 1s (`traversal_cooldown_duration`); gratis, no toca taps ni cooldown de combate. Knobs ya existen en `arm_tuning.tres`. Falta headless + iterar jugando montos/cadencia de combate y rango/duración/cooldown de traversal (nada de esto se probó en engine todavía). Agarres/objetos genéricos de traversal siguen pendientes, fase siguiente |
