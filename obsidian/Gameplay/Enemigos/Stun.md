...
- **Se para**: tras `ragdoll_getup_delay` (0.5 s, el "se para en X") y solo cuando ya vencio cualquier stun vigente, el ragdoll se congela, el cuerpo se reubica donde rodo y se endereza con un tween de `ragdoll_stand_time`. La reubicacion pasa por `_restore_body_from_ragdoll`: toma X/Z del `RigidBody` y la **Y de un raycast al piso real** (`_floor_y_below`, mask `LAYER_WORLD`), no la altura de despegue. Asi, si el cuerpo rodo por un borde a otro nivel, se para donde quedo y no salta de vuelta a la altura desde la que lo lanzaron. *(2026-07-15, reemplaza el snap a `_airborne_ground_y`)*
- **El ragdoll es un estado quebrado: se lo puede juggle**. Como todo ragdoll viene de un stun o un push (ambos ya quebraron la reserva), el cuerpo caido cuenta como quebrado en todos los gates: `_breaks_poise`, `try_apply_stun` y la pausa del poise tratan `_ragdolling` igual que `is_stunned()`. Un `launch` sobre un ragdoll **lo interrumpe** (`_interrupt_ragdoll`): el `CharacterBody` vuelve a mandar en la posicion donde quedo el `RigidBody`, se marca `STUNNED` y sale volando de nuevo. El stun del mismo golpe (que cae sincronico en `receive_hit`) entra directo y sostiene el juggle. *(2026-07-15)*
- Durante el ragdoll un golpe **sin launcher** extiende el timer, congela el `RigidBody3D` y mantiene el feedback amarillo (no lo levanta: solo el `launch` interrumpe). El **daño siempre entra** por `Hurtbox.receive_hit → Health`.
