...
- [/] Animacion del player sobre el maniqui UAL: `PlayerAnimationController` (locomocion Idle/Walk/Sprint, ninja jump, wall slide con rotacion a la pared, stun con pose congelada, arma en mano via BoneAttachment3D) + clips por golpe de Espada/Mazo via `WeaponBase.visual_clip_started`, escalados a la duracion mecanica; probe verde (`probe_animaciones_player`, 30 fases). **BUG confirmado jugando: el maniqui mira AL REVES (180°) en locomocion, salto y espada** — el esqueleto UAL (estilo Unreal) mira +Z y el forward del Player es -Z; arreglo probable: rotar `Visual` 180° en Y (y re-verificar `face_wall` del wall slide, que compensaba sobre la orientacion invertida). **Ademas el wall slide se ve HORIZONTAL y debe ser vertical**: los clips `Slide_*` de UAL2 son un derrape de piso (acostado); rotar la pose para deslizar de pie contra el muro o reemplazar por una pose vertical propia. Despues: tunear grip del arma en mano (`hand_attach_*`), umbrales/blend, tramos de stun, y re-ubicar el fogonazo de poise-chip — [[Player]] [[Animacion Espada]] [[Animacion Mazo]] [[Animacion]]
- [ ] Animacion enemiga — mismos pendientes que el player, confirmados jugando: (1) el maniqui enemigo mira 180° AL REVES (esqueleto UAL +Z vs forward -Z; rotar su `Visual` junto con el fix del player) y (2) la espada del enemigo NO esta anclada a la mano — los clips de combate animan con la mano vacia; replicar la opcion A del player (`BoneAttachment3D` en `hand_r` + mesh del arma), sin olvidar el maniqui aparte del ragdoll (`UAL2_Ragdoll`) — [[Animacion]] [[Enemigos]]
