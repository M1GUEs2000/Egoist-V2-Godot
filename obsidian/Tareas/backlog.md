...
- [t] EVADE enemigo (esquive humano): **implementado 2026-07-13** — receptor del telegraph + gates (percepcion/trayectoria/estado/cooldown/dado con extremos deterministas) + delay de reaccion + intent STRAFE + GroundLocomotion.strafe, segun los 5 principios de [[Comportamientos]] > "EVADE — implementado"; sin i-frames enemigos. Falta validar headless/smoke y tunear jugando chance/cooldown/duracion/rango — [[Comportamientos]] [[IA]] [[Stun]]
- [t] Engage en rango (reposicionamiento proactivo): **implementado 2026-07-13** — cadencia entre combos (attack_pause_min/max) + orbita al target al ring (strafe_ring_fraction) en vez de plantarse enfrente esperando el cooldown; se adelanto respecto del diseño original de EVADE (estaba diferido). Falta validar headless/smoke y tunear jugando pausas/ring/strafe_speed — [[Comportamientos]] [[IA]]
- [ ] Espejo LimboAI de engage + EVADE: hojas limbo_engage_target (reemplaza limbo_start_attack, borrada) y limbo_evade_window ya en el arbol del builder, pero el backend LIMBO completo sigue sin validar en Godot — [[IA]]
