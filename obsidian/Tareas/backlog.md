...
- [ ] Camara que rota: el jugador mueve la camara horizontalmente hacia la izquierda o derecha con el stick, pero se mantiene mirando al personaje (no permite colocarse completamente detras de el, solo desviacion lateral hasta un maximo de ±30° desde la posicion central). Al soltar el stick durante uno o dos segundos, vuelve suavemente al centro. La posicion central varia por area pero el movimiento se mantiene igual (`CameraRig`, ver METODOLOGIA.md)
- [ ] Enemigo con timer que cambia de mundo y deja una grieta temporal: trigger ON_TIMER (no ON_HIT/ON_DEATH); al cambiar de mundo deja una grieta cruzable donde estaba, con ventana de tiempo antes de cerrarse — [[World Switch]] [[Afiliacion de Mundo]]
- [ ] Enemigo con proyectil de world switch: variante de `RangedAttack`/`Projectile` que cambia el mundo al impactar — [[World Switch]] [[Ataques Enemigos]]
- [ ] EVADE enemigo (esquive humano): receptor del telegraph + gates (percepcion/trayectoria/estado/cooldown/dado) + delay de reaccion + intent STRAFE + GroundLocomotion.strafe; diseño de 5 principios cerrado en [[Comportamientos]] > "EVADE — diseño acordado"; sin i-frames enemigos en el primer pase — [[Comportamientos]] [[IA]] [[Stun]]
