...
- [ ] Corregir la propiedad de las señales de Mover: al iniciar un recorrido nuevo, `mover_cancelled(SUPERSEDED)` del recorrido reemplazado no debe quemar los slots diferidos ni cerrar el bookkeeping del `AttackMovementProfile` nuevo — [[Mover y Floater]] [[Espada]] [[Combate]]
- [ ] Hacer que `PlayerCombat.cancel_input()` descarte el press/cargado sin ejecutarlo al recibir stun o volver al spawn; hoy delega en `InputBuffer.release()` y puede disparar el hold cancelado — [[Input Feel]] [[Stun]] [[Combate]]
- [ ] Impedir que se empiece a cargar otro ataque mientras sigue ejecutándose un cargado; además, proteger el cierre tardío de la Y cargada aérea para que una rutina anterior no apague el estado de otra — [[Input Feel]] [[Espada]] [[Combate]]
...
- [t] Verificacion headless del batch (--import, --quit-after 2) — [[Arquitectura Godot]]
...
- [/] Poise (stagger): correr headless del medidor nuevo — [[Stun]] [[Combate]]
...
- [/] Enemigo con proyectil de world switch: variante de `RangedAttack`/`Projectile` que cambia el mundo al impactar al JUGADOR (`world_switch_on_player_hit` en `Projectile`); falta headless y armar el `projectile_scene` con el flag activo — [[World Switch]] [[Ataques Enemigos]]
...
- [/] Animacion enemiga — RESUELTOS el maniqui al reves (180° en `UAL2_Standard`) y la espada sin anclar (opcion A: `BoneAttachment3D` en `hand_r` con copia del `BladeMesh`, arma orbital invisible con hitbox intacto, por codigo en `EnemyAnimationController` asi lo heredan todos los prefabs); probe verde (`probe_animaciones_ia`, fase `arma_en_mano`). Falta re-validar jugando y tunear el grip (`hand_attach_*`) — [[Animacion]] [[Enemigos]]
- [t] Estela de la Espada: `SwordTrail` procedural (tira de `ImmediateMesh` muestreada del hueso `hand_r`, gradiente + erosion por ruido). Se prende y apaga con la ventana de dano de la hoja, asi que sale sola en los dos combos, los cuatro taps, el X cargado y la Y cargada. Falta tunear `sword_trail_tuning.tres` jugando: `lifetime` (es el largo del arco, y por encima del largo del golpe la estela se ve desprendida de la hoja), `max_segment_length` contra la velocidad real del swing, y el gradiente — hoy va blanco a azul y convive con un glow de carga naranja — [[Trail]] [[VFX]] [[Espada]]
- [ ] H3: cambiar la estela procedural de la Espada por una **slash card** — mesh de arco autorado con UV espejada y textura channel-packed, en las capas base / highlight / support. Es el intercambio inverso al de hoy: la card deja DIRIGIR la forma del corte (no queda atada al recorrido del arma) a cambio de alinearla a mano contra cada animacion. Por eso espera a H3, cuando los ataques de Espada esten hechos en Blender y haya animacion final que valga la pena alinear. Del marco de capas, la que mas se nota y hoy no existe es el **highlight**: mas chico, mas brillante y con el taper agresivo que le corresponde a una espada — [[Trail]] [[Animacion Espada]] [[Espada]]
...
- [ ] Arreglar el tap adelante y atras: debe dispararse solo con el input del tap, no cuando el joystick pasa por esa direccion al dar una vuelta — [[Input Feel]] [[Movimiento Base]]
- [ ] Agrandar la hitbox del rebote en enemigos — [[Rebote en Enemigos]]
- [t] Convertir el sprint en estado de velocidad: sostener el boton para mantenerlo y que consuma meter mientras dura, en vez de cargar y quedar activo gratis. HECHO (2026-07-28): ademas se descongelo el nivel en el aire, asi soltar apaga la carrera en pleno salto. Falta tunear jugando `sprint_meter_drain_per_second` (hoy 0.1 = 10% del meter por segundo) y sentir como quedaron las cadenas de paredes, que antes heredaban el nivel congelado del despegue — [[Sprint]] [[Meter]]
- [t] Meter por arma: la ganancia por golpe/kill salio de `PlayerTuning` y vive en el `.tres` de cada arma. Espada y Mazo quedaron con los valores viejos, asi que falta separarlos de verdad — el Mazo pega mas lento, decidir si compensa con mas ganancia por golpe o si eso es su identidad — [[Meter]] [[Espada]] [[Mazo]]
- [t] Cobrar el Y cargado terrestre (launcher) de Espada y Mazo: 1 barra, sin barra cae al tap. Falta jugar si el launcher sigue valiendo la pena a ese precio y si el Mazo deberia pagar distinto que la Espada — [[Meter]] [[Espada]] [[Mazo]]
- [t] Preview de gasto del meter en el HUD: el tramo que un cargado va a consumir late sobre las barras, con el color del glow del arma. Falta jugar el ritmo del latido y los alphas; el rediseño visual del HUD lo puede reemplazar entero — [[Meter]] [[Combate]]
- [ ] Decidir si el meter button debe cobrar dos veces: sostenerlo moviendose paga el drenaje del sprint Y el costo del tap direccional, porque son el mismo boton. Salidas: descontarle al tap lo que ya cobro el sprint, cobrar solo el mayor de los dos, o dejarlo asi si al jugarlo se siente bien — [[Meter]] [[Sprint]] [[Taps Direccionales]]
- [ ] Verificar headless los cambios de meter/sprint del 2026-07-28: se commitearon sin `--import` (no habia Godot en la maquina) — [[Meter]]
- [f] Tunear valores finales de wall slide, wall impulse y sprint — [[Wall Slide y Wall Jump]] [[Sprint]]
- [x] Fundacion tecnica Godot: proyecto 4.7, autoloads, estructura feature-based, Health/Hurtbox/Hitbox, tuning — [[Arquitectura Godot]]
