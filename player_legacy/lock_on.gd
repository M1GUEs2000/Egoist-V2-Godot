class_name LegacyLockOn extends Node3D
...
## lock activo (ver PlayerLegacy._unhandled_input; CameraRig deja de rotar libre mientras is_locked).
...
func setup(body: PlayerLegacy, cam: Camera3D) -> void:
	_body = body
	_cam = cam
...
## SIN tocar el lock activo. Lo usa PlayerLegacyLocomotion para el snap del golpe cuando no hay lock.
...
## marcador liviano del Brazo (ver PlayerLegacyArm) para ubicarse en el mismo punto que el reticle.
