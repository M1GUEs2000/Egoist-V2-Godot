class_name PlayerLegacyLocomotion extends Node
## Bloque (ex PlayerLegacyLocomotion.cs): movimiento por input relativo a cámara + attack lunge
## (avance corto en cada golpe). Expone los helpers de dirección (camera_relative /
## movement_direction) que reusan dash y swing. El snap hacia el target lockeado (LegacyLockOn)
## ajusta movimiento y encare del golpe; el reticle en sí lo dueña LegacyLockOn.
