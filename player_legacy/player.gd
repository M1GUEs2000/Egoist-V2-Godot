class_name PlayerLegacy extends CharacterBody3D
...
@onready var locomotion: PlayerLegacyLocomotion = $Locomotion
@onready var dash: PlayerLegacyDash = $Dash
@onready var launcher: PlayerLegacyLauncher = $Launcher
@onready var wall_slide: PlayerLegacyWallSlide = $WallSlide
@onready var floor_slide: PlayerLegacyFloorSlide = $FloorSlide
@onready var enemy_bounce: PlayerLegacyEnemyBounce = $EnemyBounce
@onready var air_kill_reset: PlayerLegacyAirKillReset = $AirKillReset
@onready var stun: PlayerLegacyStun = $Stun
@onready var meter: PlayerLegacyMeter = $Meter
@onready var health: Health = $Health
@onready var player_health: PlayerLegacyHealth = $PlayerLegacyHealth
@onready var combat: PlayerLegacyCombat = $Combat
@onready var action_world_switch: ActionWorldSwitchModifier = $ActionWorldSwitchModifier
@onready var lock_on: LegacyLockOn = $LegacyLockOn
@onready var arm: PlayerLegacyArm = $Arm
...
func receive_stun(stun_settings: StunSettings, mode := PlayerLegacyStun.Mode.STILL,
...
func try_apply_stun(duration: float, poise_damage: float, mode := PlayerLegacyStun.Mode.STILL,
...
func apply_stun(duration: float = -1.0, mode := PlayerLegacyStun.Mode.STILL,
		push_direction := Vector3.ZERO, horizontal_speed := 0.0, vertical_speed := 0.0,
...
## Programa un impulso que PlayerLegacyDash aplica solo al terminar naturalmente el dash actual.
...
func _tick_stunned(delta: float) -> void:
	...
	if is_on_floor():
		...
		air_state = AirState.AIRBORNE
	if stun.mode == PlayerLegacyStun.Mode.PUSH:
...
func _on_stunned_started(_duration: float, _mode: PlayerLegacyStun.Mode) -> void:
	if _mesh == null:
		return
	if _chip_tween != null and _chip_tween.is_valid():
