...
@export var passive_memory := 10.0
@export var reactive_memory := 20.0
@export var aggressive_memory := 40.0
@export var ultra_aggressive_memory := 60.0
...
func tick(new_target: Node3D, hostility: int, can_chase: bool) -> void:
	...
	if sees and target != null:
		...
		_search_until = World.now() + _memory_for(hostility)
...
func _memory_for(hostility: int) -> float:
	match hostility:
		EnemyBase.Hostility.PASSIVE:
			return passive_memory
		EnemyBase.Hostility.REACTIVE:
			return reactive_memory
		EnemyBase.Hostility.AGGRESSIVE:
			return aggressive_memory
		EnemyBase.Hostility.ULTRA_AGGRESSIVE:
			return ultra_aggressive_memory
	return aggressive_memory
