...
func is_total() -> bool:
	return _active and _settings != null and _settings.mode == MoverSettings.Mode.TOTAL

func is_partial() -> bool:
	return _active and not is_total()

func blocks_jump() -> bool:
	return is_partial() and _dir.y < -0.0001
...
func tick(delta: float) -> void:
	if not _active or _body == null or delta <= 0.0:
		return
	var frame_speed := _prepare_step(delta)
	...
	if _partial_will_reach or _traveled >= _settings.distance:
...
## Paso parcial para el Player: el Mover decide solo Y y el cuerpo conserva su propio
## move_and_slide, contactos y velocidad horizontal. Se usa para plunge/hop, nunca para trayectorias
## diagonales ni horizontales.
func apply_partial_vertical(delta: float, current_vertical_velocity: float) -> float:
	if not is_partial() or delta <= 0.0:
		return current_vertical_velocity
	var frame_speed := _prepare_step(delta)
	return _dir.y * frame_speed

func finish_partial_vertical(before: Vector3) -> void:
	if not is_partial() or _body == null:
		return
	_traveled += maxf(0.0, (_body.global_position - before).dot(_dir))
	if _partial_will_reach or _traveled >= _settings.distance:
		_finish(FinishReason.DISTANCE)
...
func _prepare_step(delta: float) -> float:
	_speed += _settings.acceleration * delta
	if _speed < 0.0:
		_speed = 0.0
	var remaining := maxf(0.0, _settings.distance - _traveled)
	var frame_speed := _speed
	_partial_will_reach = frame_speed * delta >= remaining
	if _partial_will_reach:
		frame_speed = remaining / delta
	return frame_speed
